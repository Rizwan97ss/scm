...
- `tests/Feature/Exams` — `GradingScaleTest` (CRUD, band replacement), `ExamTest` (create-with-subjects, upsert-not-replace on subject edits, teacher-can/cannot-enter-marks, marks-cannot-exceed-max, report-card grade computation, publish-gated visibility for Student/Parent, cross-school isolation), `OnlineExamTest` (question CRUD with exactly-one-correct-option validation, teacher-can/cannot-configure, student auto-graded attempt end-to-end, section/max-attempts/resubmit eligibility rules, attempt-privacy denial), `TermResultTest` (weighted-average computation, section-rank, parent visibility)
