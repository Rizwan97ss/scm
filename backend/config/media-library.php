...
return [
     ...
     *
     * Defaults to the PRIVATE 'local' disk (storage/app/private — not
     * symlinked to public/storage), not 'public'. Every upload here is
     * genuinely sensitive by default across this app's actual collections
     * (student photos/birth certificates/report cards, homework
     * attachments, staff avatars) — access is enforced by
     * MediaController::show()'s per-model Policy check, not by the URL
     * being hard to guess. A collection that's deliberately meant to be
     * world-readable (there are none today) should override its own disk
     * explicitly in that model's registerMediaCollections(), not flip this
     * global default back to 'public'.
     */
