...
class LibraryTest extends TestCase
{
    ...
    public function test_librarian_cannot_view_another_schools_book_by_guessing_its_id(): void
    {
        $schoolA = $this->createSchool();
        $librarianA = $this->createUserWithRole($schoolA, 'Librarian');

        $schoolB = $this->createSchool();
        tenancy()->initialize($schoolB);
        $bookB = Book::factory()->create();

        $response = $this->actingAsInSchool($librarianA)->getJson("/api/v1/books/{$bookB->id}");
