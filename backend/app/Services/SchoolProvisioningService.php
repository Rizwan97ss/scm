...
class SchoolProvisioningService
{
    private const SCHOOL_SCOPED_ROLE_PERMISSIONS = [
        ...
        'Class Teacher' => [
            ...
            'student-attendance.view', 'student-attendance.mark', 'student-attendance.edit', 'student-attendance.export',
            'exams.view',
            // No exam-marks.publish: results are declared by School Admin/
            // Principal only — Class Teacher enters and edits marks for their
            // section, but a second person always signs off (ExamController::
            // publishGroup) before students/parents can see them.
            'exam-marks.view', 'exam-marks.enter', 'exam-marks.edit', 'exam-marks.export', 'exam-marks.import',
            'questions.view', 'questions.create', 'questions.edit', 'questions.import',
            'online-exams.view', 'online-exams.configure',
            'homework.view', 'homework.create', 'homework.edit', 'homework.delete', 'homework.grade',
            'remarks.view', 'remarks.create', 'remarks.edit', 'remarks.delete',
    ...
    {
        // Instantiated before the try, not created via ::create() inside it:
        // the tenant database is provisioned synchronously off the model's
        // own 'created' event (School::$dispatchesEvents, no queue worker in
        // this app — see this method's docblock), so a failure there throws
        // from inside save() itself. $school needs to already be a real
        // reference (with its primary key populated, which Eloquent sets
        // right after the INSERT, before 'created' fires) for the catch
        // block's teardown() call below to have something to clean up.
        $school = new School([...$schoolAttributes, 'plan_id' => $plan->id]);
