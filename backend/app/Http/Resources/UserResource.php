...
class UserResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        return [
            ...
            // Always included, not whenLoaded-gated — same rationale as
            // `permissions` above: any call site that returns a UserResource
            // without eager-loading studentProfile would otherwise silently
            // drop this key rather than return null, and the frontend can't
            // tell "not a student" apart from "forgot to eager-load" if the
            // key just vanishes. Lets the frontend resolve "my own student
            // record" for a Student-role user (report cards, exam schedule)
            // without a dedicated lookup route — null for every other role.
