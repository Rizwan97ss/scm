...
class Exam extends Model
{
    ...
    public function scopeVisibleTo(Builder $query, User $user): Builder
    {
        if ($user->hasRole('Student')) {
            ...
            return $query
                // Phase 16: an exam is listable once it's wholly published, OR
                // at least one of its subject groups in the student's own
                // section has been individually declared, OR at least one
                // component in that section has a scheduled exam_date — the
                // last of those is deliberately publish-independent: a
                // student should be able to see an exam is coming up as soon
                // as it's scheduled, without that also exposing marks/grades,
                // which stay masked separately (SubjectResultService/
                // ExamService::reportCard()) regardless of this list-level
                // visibility. Matches ParentPortalController::childExams()'s
                // identical composition, so the two portals never disagree.
                ->where(function ($q) use ($sectionId) {
                    $q->where('is_published', true)
                        ...
                        ->orWhereHas('examSubjects', fn ($q2) => $q2->where('section_id', $sectionId)->whereNotNull('exam_date'));
                })
                ->when(
                    $sectionId,
            ...
            return $query
                ->where(function ($q) use ($sectionIds) {
                    $q->where('is_published', true)
                        ...
                        ->orWhereHas('examSubjects', fn ($q2) => $q2->whereIn('section_id', $sectionIds)->whereNotNull('exam_date'));
                })
