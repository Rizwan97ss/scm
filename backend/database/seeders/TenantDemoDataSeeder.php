...
/**
 * Populates one tenant's database with a full, realistic spread of demo data
 * across every module -- meant to be run inside `$school->run(fn () => ...)`
 * (see SchoolProvisioningService::provision()), never against the landlord
 * connection. Unlike the older AcademicStructureDemoSeeder/DemoStudentSeeder
 * (both pre-tenancy-conversion and left as known-broken placeholders), this
 * never passes 'school_id' -- tenant tables don't have that column anymore
 * (Sub-phase D stripped it; only stale #[Fillable] metadata still lists it
 * on some models, which is dead weight, not a real column).
 */
class TenantDemoDataSeeder extends Seeder
{
    ...
    /** @var array<int, string> grade level names in ascending sequence order — overridable per call, see run(). */
    private array $gradeLevelDefs = ['Kindergarten', 'Grade 1', 'Grade 2', 'Grade 3', 'Grade 4', 'Grade 5'];

    /**
     * @param  array<int, string>|null  $gradeLevelDefs  Overrides the default
     *   Kindergarten-through-Grade-5 spread — e.g. a Jr/Sr College school
     *   wants an 11th/12th tier a generic K-5 demo school doesn't. Every
     *   downstream method (sections, subjects-per-section, students,
     *   exams, ...) already derives its grade/section list from
     *   $this->gradeLevels rather than a hardcoded count, so this is the
     *   only thing that needs to change to reshape the whole demo dataset.
     */
    public function run(School $school, ?array $gradeLevelDefs = null): void
    {
        $this->school = $school;
        if ($gradeLevelDefs !== null) {
            $this->gradeLevelDefs = $gradeLevelDefs;
        }

        $this->command?->info("Seeding demo data for {$school->name} ({$school->slug})...");
        ...
        $this->command?->info("Done seeding {$school->name}.");
    }
    ...
    private function academicStructure(): void
    {
        ...
        foreach ($this->gradeLevelDefs as $sequence => $name) {
    ...
    private function staffUsers(): void
    {
        ...
        foreach ($designationDefs as $name) {
            $designations[$name] = Designation::query()->create(['name' => $name, 'description' => "{$name} at {$this->school->name}", 'is_active' => true]);
        }

        // The School Admin already exists -- SchoolProvisioningService::provision()
        // creates it before this seeder ever runs (it's the account the signup
        // handoff token is issued for). Reuse it rather than creating a second,
        ...
        foreach ($roster as $handle => $def) {
            ...
            $last = fake()->lastName();
            $email = Str::slug("{$first}.{$last}").'@'.$this->school->slug.'.test';
        ...
        $this->command?->info("  Staff seeded (password: \"password123\"): admin@{$this->school->slug}.test");
    }
        ...
        foreach ($this->sections as $section) {
            for ($i = 0; $i < 4; $i++) {
                ...
                $student = Student::query()->create([
                    'admission_number' => $idGenerator->generate($this->school, $admissionDate),
                    'first_name' => $first,
                    'last_name' => $last,
                    'gender' => $gender,
    ...
    private function gradingAndExams(): void
    {
        ...
        // One "Written" component per subject-in-section — the demo data
        // only needs one gradable component to exercise the report-card/
        // term-result pipeline end to end; SubjectResultService sums
        // however many components a group has, so this isn't a schema
        // limitation, just the simplest fixture that still exercises it.
        $writtenComponentType = AssessmentComponentType::query()->where('code', 'written')->first();
        ...
        foreach ($examDefs as $def) {
            ...
            // Every "A" section across every grade level, not a fixed 3 —
            // scales with however many grade levels this run was given
            // (gradeLevelDefs), so a 15-level Jr/Sr College school gets exam
            // data throughout, not just in whichever 3 sections happened to
            // exist in the original 6-level default spread.
            foreach (array_filter(array_keys($this->sections), fn ($key) => str_ends_with($key, '-A')) as $sectionKey) {
                ...
                foreach (['MATH', 'SCI', 'ENG'] as $subjectCode) {
                    ...
                    $examSubject = ExamSubject::query()->create([
                        ...
                        'assessment_component_type_id' => $writtenComponentType->id,
                        'subject_id' => $this->subjects[$subjectCode]->id,
                        'section_id' => $section->id,
                        'sequence' => 1,
                        'max_marks' => 100,
    ...
    {
        $sections = array_filter(array_keys($this->sections), fn ($key) => str_ends_with($key, '-A'));
    ...
    private function fees(): void
    {
        ...
        foreach (array_slice($this->students, 0, 30) as $student) {
            ...
            $invoiceNumber = $feeNumbers->nextInvoiceNumber($this->school);
            ...
            if ($amountPaid > 0) {
                Payment::query()->create([
                    'invoice_id' => $invoice->id,
                    'student_id' => $student->id,
                    'payment_number' => $feeNumbers->nextPaymentNumber($this->school),
                    'amount' => $amountPaid,
            ...
            if ($paymentOutcome === 'paid' && fake()->boolean(15)) {
                ...
                CreditNote::query()->create([
                    'invoice_id' => $invoice->id,
                    'credit_note_number' => $feeNumbers->nextCreditNoteNumber($this->school),
                    'amount' => $creditAmount,
        ...
        foreach ([now()->subMonth(), now()] as $monthDate) {
            foreach ($structures as $handle => $structure) {
                ...
                Payslip::query()->create([
                    ...
                    'payslip_number' => $payroll->nextPayslipNumber($this->school),
    ...
    private function certificates(): void
    {
        ...
        foreach (array_slice($this->students, 0, 6) as $student) {
            ...
            $content = str_replace(
                ...
                [$student->full_name, $student->admission_number, $gradeLevel?->name ?? '', $section?->name ?? '', $this->school->name, now()->toDateString()],
                $template->body
            );
            ...
            Certificate::query()->create([
                ...
                'certificate_number' => $certificates->nextCertificateNumber($this->school),
