...
/**
 * The shared LanguageSwitcher (components/layout) is styled for the
 * regular --color-* design tokens, which .marketing-scope deliberately
 * doesn't repaint (see index.css's docblock on why the marketing surface
 * is a fixed, independent identity) — its ghost-button trigger would
 * render dark-on-dark here. This is the same Dropdown primitive with a
 * trigger styled to match this page's own --mk-* palette instead.
 */
function MarketingLanguageSwitcher() {
    ...
    <Dropdown
      trigger={
        <button
          ...
          className="flex items-center text-[var(--mk-mist)] transition-colors hover:text-[var(--mk-paper)]"
        >
