  ...
  return (
    <div>
      <PageHeader
        ...
        description={`${homework.subject?.name ?? ''} · ${homework.section?.name ?? ''} · Due ${formatDate(homework.due_date)}${homework.max_score ? ` · Max score: ${homework.max_score}` : ''}`}
        breadcrumbs={[{ label: 'Homework', to: routePaths.homework }, { label: homework.title }]}
      />
            ...
            {(roster ?? []).map(({ student, submission }) => {
              ...
              return (
                <TableRow key={student.id}>
                  ...
                  <TableCell>
                    {submission ? (
                      <Badge variant={submission.status === 'graded' ? 'success' : 'info'}>{HOMEWORK_SUBMISSION_STATUS_LABELS[submission.status]}</Badge>
                    ) : (
