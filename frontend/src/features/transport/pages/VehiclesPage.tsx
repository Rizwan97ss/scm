...
export function VehiclesPage() {
  ...
  const columns: DataTableColumn<Vehicle>[] = [
    { key: 'registration_number', header: 'Registration #', sortable: true, render: (row) => <span className="font-medium">{row.registration_number}</span> },
    { key: 'capacity', header: 'Capacity', align: 'right', render: (row) => row.capacity },
    { key: 'driver_name', header: 'Driver', render: (row) => row.driver_name ?? '—' },
    { key: 'driver_phone', header: 'Phone', render: (row) => row.driver_phone ?? '—' },
    { key: 'status', header: 'Status', render: (row) => <Badge variant={row.is_active ? 'success' : 'default'}>{row.is_active ? 'Active' : 'Inactive'}</Badge> },
    {
      ...
      render: (row) => (
        <div className="flex justify-end gap-2">
          ...
          {canManage && (
            <Button variant="outline" size="sm" onClick={() => setDeleting(row)} aria-label={`Delete ${row.registration_number}`}>
  ...
  return (
    <div>
      <PageHeader
        title="Vehicles"
        description="The transport fleet."
        actions={
          canManage && (
      ...
      <Modal open={modalOpen} onOpenChange={setModalOpen} title={editing ? 'Edit Vehicle' : 'New Vehicle'}>
        <form onSubmit={onSubmit} className="flex flex-col gap-4" noValidate>
          <FormField label="Registration Number" htmlFor="registration_number" required>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Driver Name" htmlFor="driver_name">
              <Input id="driver_name" value={form.driver_name ?? ''} onChange={(e) => setForm({ ...form, driver_name: e.target.value })} />
            </FormField>
            <FormField label="Driver Phone" htmlFor="driver_phone">
      ...
      <ConfirmDialog
        ...
        title={`Delete ${deleting?.registration_number}?`}
