...
export function SignupPage() {
  ...
  const STEP_LABELS: Record<Step, string> = {
    school: t('auth.yourSchool'),
    admin: t('auth.yourAccount'),
    plan: t('auth.choosePlan'),
  }
  ...
  async function onSubmit(values: SignupFormValues) {
    ...
    try {
      ...
      if (apiError.errors) {
        ...
        setFormError(apiError.message ?? t('auth.somethingWentWrong'))
      }
    }
  }
  ...
  return (
    <AuthLayout
      eyebrow={t('auth.freeTrialEyebrow')}
      title={t('auth.setUpSchoolTitle')}
      description={t('auth.setUpSchoolDescription')}
      wide={step === 'plan'}
      footer={<p className="text-[13.5px] text-[var(--mk-ink-soft)]">{t('auth.alreadySetUp')}</p>}
    >
      ...
      <p className="mb-1 font-[var(--mk-font-mono)] text-[11px] uppercase tracking-[0.1em] text-[var(--mk-forest-deep)]">
        {t('auth.stepOf', { current: stepIndex + 1, total: STEPS.length })}
      </p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        {step === 'school' && (
          <section className="flex flex-col gap-4">
            <FormField label={t('auth.schoolName')} htmlFor="school-name" error={errors.school?.name?.message} required>
              <Input id="school-name" invalid={!!errors.school?.name} {...register('school.name')} />
            </FormField>
            <FormField
              label={t('auth.urlSlug')}
              htmlFor="school-short-name"
              error={errors.school?.short_name?.message}
              hint={t('auth.urlSlugHint')}
              required
            >
              <Input id="school-short-name" invalid={!!errors.school?.short_name} {...register('school.short_name')} />
            </FormField>
            <FormField label={t('auth.schoolEmail')} htmlFor="school-email" error={errors.school?.email?.message} hint={t('common.optional')}>
              <Input id="school-email" type="email" invalid={!!errors.school?.email} {...register('school.email')} />
            </FormField>
            <FormField label={t('common.phone')} htmlFor="school-phone" hint={t('common.optional')}>
        ...
        {step === 'admin' && (
          <section className="flex flex-col gap-4">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField label={t('auth.firstName')} htmlFor="admin-first-name" error={errors.admin?.first_name?.message} required>
                <Input id="admin-first-name" invalid={!!errors.admin?.first_name} {...register('admin.first_name')} />
              </FormField>
              <FormField label={t('auth.lastName')} htmlFor="admin-last-name" error={errors.admin?.last_name?.message} required>
                <Input id="admin-last-name" invalid={!!errors.admin?.last_name} {...register('admin.last_name')} />
              </FormField>
            </div>
            <FormField label={t('common.email')} htmlFor="admin-email" error={errors.admin?.email?.message} required>
              <Input id="admin-email" type="email" autoComplete="email" invalid={!!errors.admin?.email} {...register('admin.email')} />
            </FormField>
            <FormField label={t('auth.password')} htmlFor="admin-password" error={errors.admin?.password?.message} required>
        ...
        {step === 'plan' && (
          <section className="flex flex-col gap-4">
            ...
            {plansLoading && <p className="text-sm text-muted-foreground">{t('auth.loadingPlans')}</p>}
                  ...
                  >
                    <CardContent className="flex flex-col gap-2 pt-6">
                      ...
                      <p className="text-2xl font-semibold">
                        ...
                        <span className="text-sm font-normal text-muted-foreground">{t('auth.perMonth')}</span>
                      </p>
                      <p className="text-xs text-muted-foreground">{t('auth.trialDays', { count: plan.trial_days })}</p>
                      {plan.description && <p className="text-sm text-muted-foreground">{plan.description}</p>}
                      <ul className="mt-2 flex flex-col gap-1 text-sm text-muted-foreground">
                        <li>
                          {plan.max_students
                            ? t('auth.upToStudents', { count: plan.max_students })
                            : t('auth.unlimitedStudents')}
                        </li>
                        <li>
                          {plan.max_staff ? t('auth.upToStaff', { count: plan.max_staff }) : t('auth.unlimitedStaff')}
                        </li>
                      </ul>
                    </CardContent>
                  </Card>
        ...
        <div className="mt-8 flex items-center justify-between">
          ...
          {step === 'plan' ? (
            <Button type="submit" isLoading={isSubmitting || isRedirecting}>
              {isRedirecting ? t('auth.redirecting') : t('auth.startTrial')}
            </Button>
          ) : (
