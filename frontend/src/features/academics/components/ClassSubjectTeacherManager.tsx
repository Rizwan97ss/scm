...
export function ClassSubjectTeacherManager() {
  ...
  const invalidate = () => queryClient.invalidateQueries({ queryKey: queryKeys.classSubjectTeachers() })

  const createMutation = useMutation({
    mutationFn: (payload: ClassSubjectTeacherPayload) => classSubjectTeachersApi.create(payload),
    onSuccess: () => {
      toast.success(t('academics.teacherAssignedToast'))
      invalidate()
      setModalOpen(false)
    },
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('academics.assignmentRemovedToast'))
      invalidate()
    },
  })
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        {can('timetable.create') && (
          ...
          >
            <Plus className="h-4 w-4" /> {t('academics.assignTeacher')}
          </Button>
        )}
      </div>
      ...
      {!isLoading && activeSectionId && (assignments?.length ?? 0) === 0 && (
        <EmptyState title={t('academics.noAssignmentsYetTitle')} description={t('academics.noAssignmentsYetDescription')} />
      )}
          ...
          {assignments?.map((assignment) => (
            <li key={assignment.id} className="flex items-center justify-between gap-4 px-4 py-3">
              ...
              {can('timetable.delete') && (
                <Button variant="outline" size="sm" onClick={() => removeMutation.mutate(assignment.id)} aria-label={t('academics.removeAssignment')}>
      ...
      <Modal open={modalOpen} onOpenChange={setModalOpen} title={t('academics.assignTeacherModalTitle')} size="sm">
        <form onSubmit={onSubmit} className="flex flex-col gap-4" noValidate>
          ...
          <FormField label={t('academics.teacher')} htmlFor="teacher_id" required>
            <Select
              id="teacher_id"
