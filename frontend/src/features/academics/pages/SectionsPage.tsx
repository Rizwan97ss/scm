...
export function SectionsPage() {
  ...
  const columns: DataTableColumn<Section>[] = [
    { key: 'grade_level', header: 'Grade Level', render: (row) => row.grade_level?.name ?? '—' },
    { key: 'name', header: 'Section', sortable: true, render: (row) => row.name },
    { key: 'class_teacher', header: 'Class Teacher', render: (row) => row.class_teacher?.full_name ?? '—' },
    { key: 'room', header: 'Room', render: (row) => row.room?.name ?? '—' },
    { key: 'student_count', header: 'Students', render: (row) => row.student_count ?? '—' },
    {
      key: 'actions',
      header: '',
      align: 'right',
      render: (row) => (
        <div className="flex justify-end gap-2">
          ...
          {can('academic-structure.delete') && (
            <Button variant="outline" size="sm" onClick={() => setDeleting(row)} aria-label={`Delete section ${row.name}`}>
  ...
  return (
    <div>
      <PageHeader
        title="Sections"
        description="Class sections for each grade level within an academic year."
        actions={
      ...
      />
      <Modal open={modalOpen} onOpenChange={setModalOpen} title={editing ? 'Edit Section' : 'New Section'}>
        <form onSubmit={onSubmit} className="flex flex-col gap-4" noValidate>
          ...
          <FormField label="Grade Level" htmlFor="grade_level_id" required>
            <Select
              id="grade_level_id"
          ...
          <FormField label="Section name" htmlFor="name" required hint='e.g. "A"'>
          ...
          <FormField label="Class Teacher" htmlFor="class_teacher_id" hint="Optional">
            <Select
              id="class_teacher_id"
