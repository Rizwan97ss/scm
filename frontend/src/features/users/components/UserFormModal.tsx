...
export function UserFormModal({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('users.userCreatedToast'))
      invalidate()
      onOpenChange(false)
    },
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('users.userUpdatedToast'))
      invalidate()
      onOpenChange(false)
    },
    onError: (error) => toast.error((error as ApiError).message),
  ...
  return (
    <Modal
      ...
      title={editing ? t('common.editItem', { item: t('entities.user') }) : t('common.newItem', { item: t('entities.user') })}
      size="lg"
    >
      <form onSubmit={onSubmit} className="flex flex-col gap-4" noValidate>
        <div className="grid grid-cols-2 gap-4">
          <FormField label={t('auth.firstName')} htmlFor="first_name" required>
            <Input id="first_name" required value={form.first_name} onChange={(e) => setForm({ ...form, first_name: e.target.value })} />
          </FormField>
          <FormField label={t('auth.lastName')} htmlFor="last_name" required>
        ...
        <div className="grid grid-cols-3 gap-4">
          <FormField label={t('entities.designation')} htmlFor="designation_id" hint={t('common.optional')}>
            <Select
              id="designation_id"
              ...
              options={[{ value: 'none', label: t('users.noneOption') }, ...(designations?.data ?? []).map((d) => ({ value: String(d.id), label: d.name }))]}
            />
          </FormField>
          <FormField label={t('users.employeeIdLabel')} htmlFor="employee_id" hint={t('common.optional')}>
            <Input id="employee_id" value={form.employee_id ?? ''} onChange={(e) => setForm({ ...form, employee_id: e.target.value })} />
          </FormField>
          <FormField label={t('users.hireDateLabel')} htmlFor="hire_date" hint={t('common.optional')}>
        ...
        <FormField label={t('users.rolesColumn')} required>
        ...
        <Button type="submit" isLoading={isLoading} className="mt-2">
          {editing ? t('common.saveChanges') : t('common.createItem', { item: t('entities.user') })}
        </Button>
      </form>
    </Modal>
  )
