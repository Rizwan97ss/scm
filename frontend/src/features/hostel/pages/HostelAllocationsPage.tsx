...
export function HostelAllocationsPage() {
  ...
  const columns: DataTableColumn<HostelAllocation>[] = [
    ...
    { key: 'hostel', header: 'Hostel', render: (row) => row.hostel_room?.hostel?.name ?? '—' },
    { key: 'room', header: 'Room', render: (row) => row.hostel_room?.room_number ?? '—' },
    { key: 'bed_number', header: 'Bed', render: (row) => row.bed_number ?? '—' },
    { key: 'allocated_date', header: 'Allocated', render: (row) => formatDate(row.allocated_date) },
    { key: 'status', header: 'Status', render: (row) => <Badge variant={row.status === 'allocated' ? 'success' : 'default'}>{row.status_label}</Badge> },
    {
      key: 'actions',
      header: '',
      align: 'right',
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Which student is allocated to which room and bed."
        actions={
          canManage && (
...
function AllocateModal({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <Modal open={open} onOpenChange={onOpenChange} title="Allocate Student to Hostel">
      <form
        onSubmit={(e) => {
          e.preventDefault()
          mutation.mutate()
      ...
      >
        ...
        <FormField label="Room" htmlFor="hostel_room_id" required>
          <Select
            id="hostel_room_id"
        ...
        <FormField label="Bed Number" htmlFor="bed_number" hint="Optional">
          <Input id="bed_number" value={form.bed_number ?? ''} onChange={(e) => setForm({ ...form, bed_number: e.target.value })} />
        </FormField>
        <FormField label="Allocated Date" htmlFor="allocated_date" required>
