...
export function DataExportsList({
  ...
  const requestMutation = useMutation({
    mutationFn: request,
    onSuccess: () => {
      toast.success('Export started — this list will update once it\'s ready.')
          ...
          <CardContent className="flex items-center justify-between gap-4 pt-6">
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <Badge variant={STATUS_VARIANTS[exportItem.status]}>{exportItem.status}</Badge>
                <span className="text-sm text-muted-foreground">Requested {formatDate(exportItem.created_at)}</span>
              </div>
              {exportItem.requested_by && <p className="text-sm text-muted-foreground">by {exportItem.requested_by}</p>}
              ...
              {exportItem.status === 'ready' && exportItem.expires_at && (
                <p className="text-xs text-muted-foreground">Available until {formatDate(exportItem.expires_at)}</p>
              )}
            </div>
