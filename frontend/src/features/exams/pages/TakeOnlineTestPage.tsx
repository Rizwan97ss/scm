  ...
  if (phase === 'landing') {
    return (
      <div className="mx-auto max-w-lg">
        <PageHeader title="Online Test" />
        <Card>
          <CardContent className="flex flex-col items-center gap-4 py-10 text-center">
            <AlertTriangle className="h-8 w-8 text-warning" />
            <p className="text-sm text-muted-foreground">
              Once you start, the timer begins immediately and cannot be paused. Make sure you're ready before continuing.
            </p>
            ...
            <Card key={question.question_id}>
              <CardContent className="pt-4 sm:pt-6">
                <p className="mb-3 font-medium">
                  {index + 1}. {question.text} <span className="text-xs font-normal text-muted-foreground">({question.marks} mark{question.marks === 1 ? '' : 's'})</span>
                </p>
          ...
          <Card key={answer.question_id}>
            <CardContent className="pt-4 sm:pt-6">
              <div className="mb-1 flex items-start justify-between gap-2">
                ...
                <Badge variant={answer.is_correct ? 'success' : 'destructive'}>{answer.is_correct ? 'Correct' : 'Incorrect'}</Badge>
              </div>
              <p className="text-sm text-muted-foreground">Marks awarded: {answer.marks_awarded}</p>
