...
export function AttendanceReportPage() {
  ...
  return (
    <div>
      <PageHeader title="Attendance Report" description="Student and staff attendance trends over time." />
      ...
      {data?.student && (
        <div className="mb-8">
          <h3 className="mb-3 text-sm font-semibold">Student Attendance</h3>
          <div className="mb-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
            <StatCard label="Overall" value={data.student.overall_percentage != null ? `${data.student.overall_percentage}%` : '—'} />
          </div>
          <TrendChart trend={data.student.trend} />
          {data.student.by_section && Object.keys(data.student.by_section).length > 0 && (
            <Card className="mt-4">
              <CardContent className="pt-6">
                <h4 className="mb-2 text-xs font-medium uppercase text-muted-foreground">By Section</h4>
      ...
      {data?.staff && (
        <div>
          <h3 className="mb-3 text-sm font-semibold">Staff Attendance</h3>
          <div className="mb-4 grid grid-cols-2 gap-4 sm:grid-cols-3">
            <StatCard label="Overall" value={data.staff.overall_percentage != null ? `${data.staff.overall_percentage}%` : '—'} />
          </div>
          <TrendChart trend={data.staff.trend} />
        </div>
      )}

      {!isLoading && !data?.student && !data?.staff && <p className="text-sm text-muted-foreground">No attendance data visible to your role.</p>}
    </div>
  )
}
...
function TrendChart({ trend }: { trend: { month: string; percentage: number | null }[] }) {
  if (trend.length === 0) {
    return <p className="text-sm text-muted-foreground">No records in this range.</p>
  }
