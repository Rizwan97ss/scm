...
export function StudentDocumentUploader({ studentId }: { studentId: number }) {
  ...
  const COLLECTIONS = [
    { value: 'photo', label: t('students.docTypePhoto') },
    { value: 'birth_certificate', label: t('students.docTypeBirthCertificate') },
    { value: 'previous_report_card', label: t('students.docTypePreviousReportCard') },
    { value: 'transfer_certificate', label: t('students.docTypeTransferCertificate') },
    { value: 'other', label: t('students.docTypeOther') },
  ]
  ...
  const uploadMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('students.documentUploadedToast'))
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('students.documentRemovedToast'))
      queryClient.invalidateQueries({ queryKey: queryKeys.studentDocuments(studentId) })
    },
  })
    ...
    <div className="flex flex-col gap-4">
      {can('students.edit') && (
        <div className="flex flex-col gap-3 rounded-md border border-border p-4 sm:flex-row sm:items-end">
          <FormField label={t('students.documentTypeLabel')} className="sm:w-56">
      ...
      {!isLoading && documents?.length === 0 && <EmptyState title={t('students.noDocumentsYet')} />}
        ...
        <ul className="divide-y divide-border rounded-lg border border-border">
          {documents?.map((document) => (
            <li key={document.id} className="flex items-center justify-between gap-4 px-4 py-3">
              ...
              {can('students.edit') && (
                <Button variant="outline" size="sm" onClick={() => removeMutation.mutate(document.id)} aria-label={t('students.removeDocumentAria', { name: document.file_name })}>
