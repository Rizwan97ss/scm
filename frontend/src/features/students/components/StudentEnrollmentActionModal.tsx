...
const ACTION_COPY_KEYS: Record<EnrollmentActionType, { titleKey: string; needsTarget: boolean; confirmKey: string }> = {
  promote: { titleKey: 'students.promoteTitle', needsTarget: true, confirmKey: 'students.promoteAction' },
  transfer: { titleKey: 'students.transferTitle', needsTarget: false, confirmKey: 'students.transferConfirm' },
  withdraw: { titleKey: 'students.withdrawTitle', needsTarget: false, confirmKey: 'students.withdrawConfirm' },
  graduate: { titleKey: 'students.graduateTitle', needsTarget: false, confirmKey: 'students.graduateConfirm' },
  reactivate: { titleKey: 'students.reactivateTitle', needsTarget: true, confirmKey: 'students.reactivateAction' },
}
...
export function StudentEnrollmentActionModal({
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      if (!action) return
      if (action === 'promote') {
        if (!gradeLevelId || !sectionId || !academicYearId) throw new Error(t('students.selectGradeSectionYearError'))
      ...
      if (action === 'reactivate') {
        if (!gradeLevelId || !sectionId) throw new Error(t('students.selectGradeSectionError'))
    ...
    onSuccess: () => {
      toast.success(t('students.studentRecordUpdatedToast'))
  ...
  return (
    <Modal open={!!action} onOpenChange={onOpenChange} title={t(copy.titleKey)} size="sm">
      <form
        onSubmit={(e) => {
          e.preventDefault()
          mutation.mutate()
      ...
      >
        {copy.needsTarget && (
          <>
            <FormField label={t('entities.gradeLevel')} required>
              <Select
            ...
            <FormField label={t('entities.section')} required>
              <Select
            ...
            {action === 'promote' && (
              <FormField label={t('entities.academicYear')} required>
                <Select
        ...
        <FormField label={t('common.reason')} hint={t('students.reasonHintOptional')}>
