...
import { REMARK_CATEGORIES, REMARK_CATEGORY_LABEL_KEYS } from '@/types/homework'
...
export function StudentRemarksTab({ studentId }: { studentId: number }) {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(t('students.remarkAddedToast'))
  ...
  return (
    <div className="flex flex-col gap-4">
      {can('remarks.create') && (
        <div className="flex justify-end">
          <Button onClick={() => setModalOpen(true)}>
            <Plus className="h-4 w-4" /> {t('students.addRemarkAction')}
          </Button>
        </div>
      )}
      ...
      {!isLoading && (remarks?.data.length ?? 0) === 0 && <EmptyState title={t('students.noRemarksYet')} />}
          ...
          {remarks!.data.map((remark) => (
            <li key={remark.id} className="rounded-lg border border-border p-4">
              <div className="mb-1 flex items-center gap-2">
                ...
                {!remark.visible_to_guardian && <Badge variant="default">{t('students.staffOnlyBadge')}</Badge>}
      ...
      <Modal open={modalOpen} onOpenChange={setModalOpen} title={t('students.addRemarkAction')}>
        <form
          onSubmit={(e) => {
            e.preventDefault()
          ...
          noValidate
        >
          <FormField label={t('students.categoryLabel')} htmlFor="category">
            <Select
              id="category"
              value={form.category}
              onValueChange={(value) => setForm({ ...form, category: value as RemarkCategory })}
              options={REMARK_CATEGORIES.map((c) => ({ value: c, label: t(REMARK_CATEGORY_LABEL_KEYS[c]) }))}
            />
          </FormField>
          <FormField label={t('students.remarkLabel')} htmlFor="body" required>
            <Textarea id="body" rows={4} required value={form.body} onChange={(e) => setForm({ ...form, body: e.target.value })} />
          </FormField>
          <div className="flex items-center gap-2">
            <Checkbox checked={form.visible_to_guardian} onCheckedChange={(checked) => setForm({ ...form, visible_to_guardian: checked })} aria-label={t('students.visibleToGuardianAria')} />
            <label className="text-sm">{t('students.visibleToParentGuardianLabel')}</label>
          </div>
          <Button type="submit" isLoading={createMutation.isPending} className="mt-2">
            {t('students.addRemarkSubmit')}
          </Button>
        </form>
      </Modal>
    </div>
